 ...
 *
 * Netatmo access tokens live ~3h. `homey-oauth2app` refreshes on
 * HTTP 401, but Netatmo signals token expiry with HTTP 403 +
 * `error.code === 3`. `NetatmoOAuth2Client.onShouldRefreshToken`
 * extends the trigger to cover that case, so refresh is fully
 * automatic here. All network errors are caught and logged without
